...
const fieldGroups = [
  {
    ...
    icon: UserRound,
    fields: [
      { name: "workerName", label: "Nama pekerja", placeholder: "Isi nama lengkap pekerja" },
  ...
  },
  {
    title: "Pekerjaan",
    icon: Save,
    fields: [{ name: "jobSection", label: "Bagian pekerjaan", type: "select", options: jobSectionOptions }]
  }
];
    ...
    <div className="space-y-4">
      <section className="official-panel rounded-lg border border-line bg-white p-4 shadow-sm">
        ...
        <div className="grid gap-3 text-sm text-slate-600 sm:grid-cols-2">
          ...
          <InfoTile label="Tanggal perjanjian" value={formatDateLong(currentDate)} />
          <InfoTile label="Tempat dibuat" value="PASURUAN" />
          <InfoTile label="Perusahaan" value={companyDefaults.companyName} />
          <InfoTile label="Diwakili oleh" value={companyDefaults.companyRepresentative} />
        </div>
      </section>
      ...
      {fieldGroups.map((group) => {
        ...
        return (
          <section key={group.title} className="official-panel rounded-lg border border-line bg-white p-4 shadow-sm">
...
function SignaturePad({ form, pendingSignature, onPendingSignature, setStatus }) {
  ...
  const displayedSignature = pendingSignature || (form.signatureImageUrl ? `${API_ORIGIN}${form.signatureImageUrl}` : "");
    ...
    <section className="official-panel rounded-lg border border-line bg-white p-4 shadow-sm">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">Gambar tanda tangan pekerja pada area berikut.</p>
        </div>
        <Status value={pendingSignature || form.eSignatureStatus === "signed" ? "signed" : "not_started"} />
      </div>
      <canvas
        ref={canvasRef}
        width="720"
...
function FaceCapture({ form, pendingFace, onPendingFace, setStatus }) {
  ...
  const capture = () => {
    ...
    onPendingFace({ imageData, consent, captureNote: "Captured inside browser face frame" });
    setStatus("Foto wajah siap. Tekan Submit untuk menyimpan ke database.");
  };

  const displayedFace = pendingFace?.imageData || (form.faceImageUrl ? `${API_ORIGIN}${form.faceImageUrl}` : "");
    ...
    <section className="official-panel rounded-lg border border-line bg-white p-4 shadow-sm">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">Ambil foto wajah pekerja dalam frame verifikasi.</p>
        </div>
        <Status value={pendingFace ? "captured" : form.faceVerificationStatus} />
      </div>
      <div className="relative aspect-[4/3] overflow-hidden rounded-md bg-slate-900">
        ...
        {!isCameraOn && (
          <div className="absolute inset-0 grid place-items-center text-sm font-bold text-white">Kamera belum aktif</div>
        )}
      </div>
      <canvas ref={canvasRef} className="hidden" />
      <label className="mt-3 flex items-start gap-2 text-xs text-slate-600">
        <input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} />
        Saya menyetujui penyimpanan foto wajah untuk kebutuhan verifikasi perjanjian kerja.
      </label>
      <div className="mt-3 flex flex-wrap gap-2">
        <button className="btn-secondary" type="button" onClick={isCameraOn ? stopCamera : startCamera}>
          <Camera size={16} />
          {isCameraOn ? "Matikan Kamera" : "Aktifkan Kamera"}
        </button>
        <button className="btn-primary" type="button" onClick={capture} disabled={!isCameraOn}>
...
function Status({ value }) {
  ...
  return (
    <span className={`rounded-md px-2 py-1 text-xs font-bold ${done ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>
...
function Fill({ children }) {
  return <span className="border-b border-dotted border-slate-500 px-1 font-semibold text-ink">{children || "................"}</span>;
}
...
function DocumentPreview({ data, pendingSignature, pendingFace }) {
  ...
  return (
    <article className="document-page mx-auto min-h-[1120px] w-full max-w-[794px] bg-white px-12 py-12 text-[13px] leading-6 text-slate-900 shadow-panel">
      <h1 className="mb-6 text-center text-base font-bold uppercase underline">Surat Perjanjian Kerja Harian Lepas</h1>
      <p>
        Pada hari ini <Fill>{day}</Fill> tanggal <Fill>{made.date}</Fill> bulan <Fill>{made.month}</Fill> tahun{" "}
        <Fill>{made.year}</Fill>, bertempat di <Fill>PASURUAN</Fill>, telah dibuat dan disepakati Perjanjian Kerja
        Harian Lepas antara:
      </p>
      <SectionTitle>PIHAK PERTAMA (PERUSAHAAN)</SectionTitle>
      <IdentityRow label="Nama Perusahaan" value={companyDefaults.companyName} />
      <IdentityRow label="Alamat" value={companyDefaults.companyAddress} />
      <IdentityRow label="Diwakili oleh" value={companyDefaults.companyRepresentative} />
      ...
      <IdentityRow label="Tempat/Tanggal Lahir" value={`${data.workerBirthPlace || "................"} / ${formatDateLong(data.workerBirthDate)}`} />
      ...
      <p className="mt-4">Kedua belah pihak sepakat mengikatkan diri dalam Perjanjian Kerja Harian Lepas dengan ketentuan sebagai berikut:</p>
      <Article number="1" title="Jenis Pekerjaan">
        PIHAK KEDUA bekerja sebagai tenaga kerja harian lepas pada bagian <Fill>{data.jobSection}</Fill> sesuai kebutuhan operasional perusahaan.
      </Article>
      <Article number="2" title="Sistem Kerja dan Upah">
        <ol className="list-decimal space-y-1 pl-5">
          <li>Hubungan kerja ini bersifat Harian Lepas.</li>
          <li>PIHAK KEDUA bekerja berdasarkan kebutuhan pekerjaan dari PIHAK PERTAMA.</li>
          <li>Upah kerja diberikan sesuai kesepakatan kedua belah pihak dan kebijakan perusahaan.</li>
          <li>Pembayaran upah dilakukan secara harian sesuai kebijakan perusahaan.</li>
          <li>PIHAK KEDUA menyetujui adanya potongan administrasi / CV sebesar Rp 15.000 setiap 1 bulan sekali untuk pengurusan administrasi BPJS Ketenagakerjaan program JKK dan JKM.</li>
        </ol>
      </Article>
      <Article number="3" title="Jam Kerja dan Lembur">PIHAK KEDUA bersedia bekerja dengan sistem 3 shift sesuai kebutuhan operasional perusahaan, termasuk kerja lembur apabila dibutuhkan perusahaan.</Article>
      <Article number="4" title="Kewajiban Pekerja">PIHAK KEDUA wajib mematuhi peraturan perusahaan, menjaga ketertiban, kedisiplinan, nama baik perusahaan, dan bertanggung jawab atas kelalaian pribadi sesuai ketentuan yang berlaku.</Article>
      <Article number="5" title="Keselamatan dan Kecelakaan Kerja">Apabila terjadi kecelakaan kerja, penanganan dan pembiayaan ditanggung melalui BPJS Ketenagakerjaan sesuai syarat dan ketentuan yang berlaku.</Article>
      <Article number="6" title="Berakhirnya Hubungan Kerja">Hubungan kerja harian lepas ini dapat berakhir sewaktu-waktu sesuai kebutuhan operasional perusahaan. Karena bersifat harian lepas, PIHAK KEDUA tidak berhak atas pesangon setelah hubungan kerja berakhir.</Article>
      <Article number="7" title="Penyelesaian Perselisihan">Segala perselisihan diselesaikan terlebih dahulu secara musyawarah antara PIHAK PERTAMA dan PIHAK KEDUA, lalu mengikuti ketentuan hukum dan peraturan ketenagakerjaan yang berlaku apabila diperlukan.</Article>
      <Article number="8" title="Penutup">Demikian Surat Perjanjian Kerja Harian Lepas ini dibuat dengan sebenar-benarnya dalam keadaan sadar, sehat jasmani dan rohani, tanpa paksaan dari pihak manapun, dan disetujui oleh kedua belah pihak.</Article>
      ...
      <div className="mt-10 grid grid-cols-2 gap-8 text-center">
        <div>
          ...
          <div className="h-20" />
          <p className="font-semibold">({companyDefaults.companyRepresentative})</p>
        </div>
        <div>
          ...
          <div className="flex h-20 items-center justify-center">
            {(pendingSignature || data.signatureImageUrl) && (
              <img
                src={pendingSignature || `${API_ORIGIN}${data.signatureImageUrl}`}
...
function ChangeRequestPage({ setPage, setStatus }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setForm({ workerName: "", workerKtp: "", workerPhone: "", requestedChanges: "", reason: "" });
    ...
    <main className="mx-auto max-w-3xl px-5 py-5">
      <section className="official-panel rounded-lg border border-line bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Gunakan form ini jika terdapat salah input pada data perjanjian yang sudah dikirim.
        </p>
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <Field field={{ name: "workerName", label: "Nama pekerja" }} value={form.workerName} onChange={update} />
          <Field field={{ name: "workerKtp", label: "No. KTP" }} value={form.workerKtp} onChange={update} />
          <Field field={{ name: "workerPhone", label: "No. HP" }} value={form.workerPhone} onChange={update} />
          <Field
            field={{ name: "requestedChanges", label: "Data yang ingin diubah", multiline: true }}
            value={form.requestedChanges}
            onChange={update}
          />
          <Field field={{ name: "reason", label: "Alasan", multiline: true }} value={form.reason} onChange={update} />
        </div>
        <div className="mt-5 flex flex-wrap gap-2">
          <button className="btn-secondary" type="button" onClick={() => setPage("form")}>
            Kembali
          </button>
          <button className="btn-primary" type="button" onClick={submit} disabled={isSubmitting}>
      ...
      <section className="official-panel w-full rounded-lg border border-line bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Akses internal CV. KRAINING MULTI ABADI.</p>
          </div>
        </div>
        <div className="space-y-3">
          ...
          <Field
            ...
            onChange={(name, value) => setCredentials((current) => ({ ...current, [name]: value }))}
          />
          <button className="btn-primary w-full" type="button" onClick={submit} disabled={isSubmitting}>
  ...
  return (
    <main className="mx-auto grid max-w-[1500px] gap-5 px-5 py-5 lg:grid-cols-[260px_1fr]">
      <aside className="official-panel rounded-lg border border-line bg-white p-4 shadow-sm">
        <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-gold">Admin Panel</p>
        <h2 className="mt-1 text-base font-black">CV. KRAINING MULTI ABADI</h2>
        <p className="mt-1 text-xs text-slate-500">Login sebagai {adminUser?.username}</p>
            ...
            return (
              <button
                ...
                className={`flex h-10 w-full items-center gap-2 rounded-md px-3 text-sm font-bold ${
                  adminTab === tab.id ? "bg-navy text-white" : "text-slate-600 hover:bg-slate-100"
                }`}
              >
                <Icon size={16} />
                {tab.label}
        ...
        <button className="btn-secondary mt-5 w-full" type="button" onClick={logoutAdmin}>
          <LogOut size={16} />
          Keluar
        </button>
      </aside>
      <section className="min-w-0">
        ...
        {adminTab === "changes" && (
          <ChangeRequestsAdmin changeRequests={changeRequests} reviewChangeRequest={reviewChangeRequest} />
        )}
      </section>
    </main>
  );
  ...
  return (
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {cards.map(([label, value]) => (
        <div key={label} className="official-panel rounded-lg border border-line bg-white p-5 shadow-sm">
  ...
  return (
    <section className="official-panel rounded-lg border border-line bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 space-y-3">
          ...
          <div key={item.id} className="rounded-lg border border-line p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-slate-500">KTP {item.workerKtp} - {item.workerPhone}</p>
              </div>
              <Status value={item.status} />
            </div>
            <p className="mt-3 text-sm"><span className="font-bold">Perubahan:</span> {item.requestedChanges}</p>
            {item.reason && <p className="mt-1 text-sm"><span className="font-bold">Alasan:</span> {item.reason}</p>}
            {item.status === "pending" && (
              <div className="mt-3 flex flex-wrap gap-2">
                <button className="btn-primary h-9" type="button" onClick={() => reviewChangeRequest(item.id, "approved")}>
                  Setujui
                </button>
                <button className="btn-secondary h-9" type="button" onClick={() => reviewChangeRequest(item.id, "rejected")}>
                  Tolak
                </button>
              </div>
            )}
        ...
        {!changeRequests.length && <p className="text-sm text-slate-500">Belum ada pengajuan perubahan.</p>}
      </div>
    </section>
  );
}

function AdminArchivePage({ agreements, search, setSearch, loadAgreements, selectAgreement, deleteAgreement }) {
      ...
      <section className="official-panel rounded-lg border border-line bg-white p-5 shadow-sm">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500">CV. KRAINING MULTI ABADI - Administrasi perjanjian kerja harian lepas.</p>
          </div>
        </div>
        <form onSubmit={submitSearch} className="mb-4 flex max-w-md items-center gap-2 rounded-md border border-line px-2">
          <Search size={15} className="text-slate-400" />
          <input value={search} onChange={(event) => setSearch(event.target.value)} className="h-10 flex-1 text-sm outline-none" placeholder="Cari nama, KTP, bagian" />
        </form>
            ...
            <tbody>
              {agreements.map((item) => (
                <tr key={item.id} className="border-b border-line">
                  ...
                  <td>{item.jobSection}</td>
                  <td>{formatDateLong(item.agreementDate)}</td>
                  <td><Status value={item.eSignatureStatus} /></td>
                  <td><Status value={item.faceVerificationStatus} /></td>
                  <td className="text-right">
                    <button className="btn-secondary mr-2 h-9" type="button" onClick={() => selectAgreement(item)}>Buka</button>
                    <button className="btn-secondary h-9" type="button" onClick={() => deleteAgreement(item.id)}>
                      <Trash2 size={15} />
                    </button>
                  </td>
                </tr>
              ...
              {!agreements.length && (
                <tr>
                  <td colSpan="6" className="py-8 text-center text-slate-500">Belum ada data tersimpan.</td>
                </tr>
              )}
            </tbody>
          </table>
...
export default function App() {
  const [page, setPage] = useState(() => {
    ...
    if (window.location.hash === "#/pengajuan-perubahan") return "change-request";
    return "form";
  });
  ...
  const canSubmit = useMemo(
    () => form.workerName && form.workerBirthPlace && form.workerAddress && form.workerKtp && form.workerPhone && form.jobSection,
    [form]
  );

  const hydrateAgreement = (agreement) => ({ ...agreement, dailyWage: String(agreement.dailyWage || "") });
  ...
  useEffect(() => {
    const syncFromLocation = () => {
      ...
      else if (window.location.hash === "#/pengajuan-perubahan") setPage("change-request");
      else setPage("form");
    };
  ...
  const submitAgreement = async () => {
    ...
    try {
      ...
      let saved = selectedId ? await api.updateAgreement(selectedId, payload) : await api.createAgreement(payload);
      if (pendingSignature) {
        const signatureResult = await api.saveDigitalSignature(saved.id, pendingSignature);
  ...
  const reviewChangeRequest = async (id, reviewStatus) => {
    ...
    if (!window.confirm(`Yakin ingin ${label} pengajuan perubahan ini?`)) return;
    try {
      await api.reviewChangeRequest(id, { status: reviewStatus });
      await loadAdminData();
      setStatus(`Pengajuan perubahan berhasil di${reviewStatus === "approved" ? "setujui" : "tolak"}.`);
    } catch (error) {
      setStatus(error.message);
    }
  };
    ...
    <div className="min-h-screen bg-mist text-ink">
      <header className="sticky top-0 z-20 border-b border-navy/15 bg-white/95 shadow-sm backdrop-blur">
        <div className="mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-3 px-5 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-gold">CV. KRAINING MULTI ABADI</p>
              <h1 className="text-base font-black">Surat Perjanjian Kerja Harian Lepas</h1>
            </div>
          </div>
      ...
      {status && (
        <div className="mx-auto mt-4 max-w-[1500px] px-5">
          <div className="rounded-lg border border-amber/30 bg-amber/10 p-3 text-sm text-amber-900">{status}</div>
        </div>
      )}

      {page === "admin-login" ? (
        <AdminLoginPage onLogin={setAdminUser} setPage={setPage} setStatus={setStatus} />
      ) : page === "admin" ? (
        <AdminPage
          adminUser={adminUser}
          adminTab={adminTab}
        ...
        <main className="mx-auto grid max-w-[1500px] gap-5 px-5 pb-24 pt-5 lg:grid-cols-[minmax(420px,640px)_1fr] lg:pb-5">
          <section className="min-w-0 space-y-4">
            <section className="rounded-lg border border-navy/15 bg-navy px-5 py-4 text-white shadow-sm">
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-gold">Formulir Resmi</p>
              <h2 className="mt-1 text-lg font-black">CV. KRAINING MULTI ABADI</h2>
            ...
            <FaceCapture
              ...
              setStatus={setStatus}
            />
            <button className="btn-primary h-12 w-full" type="button" onClick={submitAgreement} disabled={!canSubmit || isLoading}>
              {isLoading ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />}
              Submit
            </button>
          </section>
          <section className="hidden min-w-0 overflow-auto rounded-lg border border-line bg-slate-200/70 p-5 lg:block">
            <DocumentPreview data={form} pendingSignature={pendingSignature} pendingFace={pendingFace} />
            <PrintPanel />
          </section>
          <button
            className="fixed bottom-4 left-4 right-4 z-30 inline-flex h-12 items-center justify-center gap-2 rounded-md bg-navy px-4 text-sm font-bold text-white shadow-lg lg:hidden"
            type="button"
          ...
          {isPreviewOpen && (
            <div className="fixed inset-0 z-50 flex flex-col bg-mist lg:hidden">
              <div className="flex items-center justify-between border-b border-line bg-white px-4 py-3 shadow-sm">
                <button className="btn-secondary h-9" type="button" onClick={() => setIsPreviewOpen(false)}>
                  Kembali
                </button>
                <p className="text-sm font-black">Preview Surat</p>
                <button className="btn-primary h-9" type="button" onClick={() => window.print()}>
                  <Printer size={15} />
                  Cetak
                </button>
              </div>
